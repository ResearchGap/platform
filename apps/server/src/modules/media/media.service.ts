...
function hasAscii(
	...
	return expected
		.split("")
		.every((value, index) => bytes[offset + index] === value.charCodeAt(0));
}
...
export class MediaService {
	...
	async upload(
		...
		if (
			...
			throw new MediaValidationError("Image must be between 1 byte and 2 MB");
		}
		if (
			!(SUPPORTED_IMAGE_MIME_TYPES as readonly string[]).includes(
				input.mimeType,
			)
		) {
			throw new MediaValidationError(
	...
	async delete(actor: AuthorizationActor, id: string): Promise<void> {
		...
		if (asset.sourceType !== MEDIA_SOURCE_TYPES.MANAGED || !asset.storageKey) {
			throw new MediaConflictError(
				"Only managed media can be deleted through this operation",
			);
		}
		if (scope === RESOURCE_SCOPES.OWNED && asset.createdById !== actor.userId) {
			throw new AuthorizationError(
				"You may only delete media that you uploaded",
			);
		}
	...
	private toDto(asset: MediaAssetRecord): MediaAssetDto {
		...
		if (!url)
			throw new MediaConflictError(
				"Media asset does not have a resolvable URL",
			);
		return {
			id: asset.id,
